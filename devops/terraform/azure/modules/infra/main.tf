variable "environment" { }   # Usually in a separate file
variable "location" { }   # called variables.tf
...
resource "azurerm_service_plan" "plan" {
  name                = "apex-${lower(var.environment)}-plan"
  location            = var.location
...
resource "azurerm_app_service" "app_service" {
  name                = "apex-${lower(var.environment)}-service"
